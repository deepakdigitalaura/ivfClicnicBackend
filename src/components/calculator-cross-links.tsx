...
export function CalculatorCrossLinks({ current, locale = "en" }: { current?: string; locale?: Locale }) {
    ...
    <section className="container-px mx-auto max-w-5xl py-10 md:py-14">
      <Reveal>
        <h2 className="text-center text-xl font-semibold text-[color:var(--plum)] md:text-2xl">
          {t("Fertility Calculators To Plan Your Pregnancy Journey")}
        </h2>
        <p className="mx-auto mt-3 max-w-xl text-center text-sm leading-relaxed text-muted-foreground">
          {t("Use our expert-designed fertility calculators to estimate IVF success rates, track ovulation, understand your fertile window, and plan your journey to parenthood with confidence.")}
        </p>
        <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {others.map((c) => (
            ...
            >
              ...
              <span className="flex-1 text-xs font-semibold leading-snug text-[color:var(--plum)]">{t(c.title)}</span>
