...
export function WhyBfiPage({ data = WHY_BFI_DEFAULTS, locale = "en" }: { data?: WhyBfiData; locale?: Locale } = {}) {
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="border-b border-border/60 bg-[color:var(--ivory)]">
        <nav className="container-px mx-auto flex max-w-[1400px] items-center gap-2 py-3 text-xs text-muted-foreground" aria-label="Breadcrumb">
          ...
          <span>/</span>
          <span className="font-medium text-[color:var(--plum)]">{t("Why BFI")}</span>
        </nav>
      </div>
          ...
          <div className="mx-auto max-w-3xl text-center">
            <Reveal>
              <span className="inline-flex items-center gap-2 rounded-full border border-[color:var(--plum)]/15 bg-white/60 px-4 py-2 text-xs font-medium uppercase tracking-[0.18em] text-[color:var(--plum)]">
                <Star className="h-3.5 w-3.5 text-[color:var(--rose)]" /> {t(hero.eyebrow)}
              </span>
            </Reveal>
            <Reveal delay={0.1}>
      ...
      <section className="py-20 lg:py-28">
        <div className="container-px mx-auto max-w-[1400px]">
          <SectionHead
            center
            eyebrow={t("Why Bavishi Fertility Institute")}
            title={<>{t("12 reasons families ")}<em className="font-display italic text-[color:var(--rose)]">{t("trust us with their dreams.")}</em></>}
            subtitle={t("For over 25 years, Bavishi Fertility Institute has been India's most trusted name in fertility care. Here's what sets us apart.")}
          />
              ...
              return (
                <StaggerItem key={i}>
                  <div className="group h-full rounded-2xl border border-border/70 bg-card p-7 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-lift">
                    ...
                    <h3 className="mt-5 text-lg font-semibold text-[color:var(--plum)]">{t(r.title)}</h3>
                    <p className="mt-2 text-[15px] leading-relaxed text-muted-foreground">{t(r.description)}</p>
                  </div>
                </StaggerItem>
              );
            })}
      ...
      <section className="bg-[color:var(--ivory)] py-20 lg:py-28">
        <div className="container-px mx-auto max-w-[1400px]">
          <SectionHead
            center
            eyebrow={t("Our Journey Since 1990")}
            title={<>{t("Three decades of ")}<em className="font-display italic text-[color:var(--rose)]">{t("pioneering firsts.")}</em></>}
            subtitle={t("From India's first ART/IVF clinic to a network of 14 centres across 8 cities — the milestones, breakthroughs, and honours that built Bavishi Fertility Institute.")}
          />
              ...
              <div key={eraIdx}>
                <Reveal>
                  <div className="mb-8 flex items-center gap-4">
                    ...
                    <span className="shrink-0 text-sm font-medium text-[color:var(--plum)]/60">{t(era.eraLabel)}</span>
                  </div>
                </Reveal>
                        ...
                        <div className="relative flex gap-6 pb-10 last:pb-0 lg:gap-8">
                          ...
                          <div className="pt-1">
                            ...
                            <div className="mt-1.5 space-y-1.5">
                              {f.items.map((item, ii) => (
                                <p key={ii} className="text-[16px] leading-relaxed text-[color:var(--plum)] font-medium">{t(item)}</p>
                              ))}
                            </div>
                          </div>
                        </div>
      ...
      <section className="bg-[color:var(--rose-soft)]/40 py-20 lg:py-28">
        <div className="container-px mx-auto max-w-[1400px]">
          <div className="mx-auto grid max-w-5xl grid-cols-1 items-center gap-12 lg:grid-cols-2">
            <div>
              <Reveal>
                <Eyebrow>{t("Ethics & Transparency")}</Eyebrow>
              </Reveal>
              <Reveal delay={0.05}>
                <h2 className="mt-4 text-3xl font-medium leading-[1.1] text-[color:var(--plum)] md:text-4xl text-balance">
                  {t("Your eggs. Your sperm. ")}<em className="font-display italic text-[color:var(--rose)]">{t("Guaranteed.")}</em>
                </h2>
              </Reveal>
              <Reveal delay={0.12}>
                <div className="mt-6 space-y-4 text-[17px] leading-relaxed text-muted-foreground">
                  <p>{t("At Bavishi Fertility Institute, ethics are not an afterthought — they are the foundation of everything we do. We are fully dedicated to the highest standards of ethical practice in fertility care.")}</p>
                  <p>{t("We discuss the pros and cons of every treatment option openly. We are completely transparent about costs, success rates, and realistic expectations. No hidden charges, no inflated promises — just honest, compassionate guidance.")}</p>
                  <p>{t("Our patients receive thorough counselling at every stage — emotional support, psychological guidance, and a team that truly cares about your wellbeing beyond just the medical outcome.")}</p>
                </div>
              </Reveal>
            </div>
                  ...
                  return (
                    <StaggerItem key={i}>
                      <div className="rounded-2xl border border-border/70 bg-card p-6 shadow-soft">
                        ...
                        <h3 className="mt-4 text-base font-semibold text-[color:var(--plum)]">{t(e.title)}</h3>
                        <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">{t(e.description)}</p>
                      </div>
                    </StaggerItem>
                  );
                })}
      ...
      <section className="py-20 lg:py-28">
        <div className="container-px mx-auto max-w-[1400px]">
          <div className="mx-auto grid max-w-5xl grid-cols-1 items-center gap-12 lg:grid-cols-2">
              ...
              <Float amplitude={6}>
                <div className="relative overflow-hidden rounded-[2rem] bg-[color:var(--plum)] p-10 text-white noise shadow-lift">
                  ...
                  <div className="relative space-y-6">
                    ...
                    <h3 className="text-2xl font-medium">{t("Class 1000 IVF Labs")}</h3>
                    <p className="text-white/70 leading-relaxed">
                      {t("Our IVF laboratories maintain air quality that is ")}<strong className="text-white">{t("10 times superior")}</strong>{t(" to the international Class 10,000 standard. This ultra-clean environment protects your embryos at every moment of their development.")}
                    </p>
                    <div className="flex items-center gap-3 rounded-xl bg-white/5 px-5 py-4 border border-white/10">
                      ...
                      <span className="text-sm text-white/80">{t("Zero cases of severe OHSS for over 10 consecutive years")}</span>
                    </div>
                  </div>
                </div>
              </Float>
            ...
            <div>
              <Reveal>
                <Eyebrow>{t("World-Class Infrastructure")}</Eyebrow>
              </Reveal>
              <Reveal delay={0.05}>
                <h2 className="mt-4 text-3xl font-medium leading-[1.1] text-[color:var(--plum)] md:text-4xl text-balance">
                  {t("Safety, quality, and ")}<em className="font-display italic text-[color:var(--rose)]">{t("precision at every step.")}</em>
                </h2>
              </Reveal>
              <Reveal delay={0.12}>
                <div className="mt-6 space-y-4 text-[17px] leading-relaxed text-muted-foreground">
                  <p>{t("Bavishi Fertility Institute operates OHSS-free clinics — we have recorded zero cases of severe Ovarian Hyperstimulation Syndrome for over a decade. Our stimulation protocols are designed for your safety as much as your success.")}</p>
                  <p>{t("Every centre is equipped with next-generation incubators, advanced cryopreservation systems, and Class 1000 clean-room IVF laboratories that meet and exceed international standards.")}</p>
                </div>
              </Reveal>
              <Reveal delay={0.18}>
            ...
            <div className="relative">
              ...
              <Reveal delay={0.1}>
                <h2 className="mx-auto max-w-3xl text-3xl font-medium leading-[1.1] md:text-4xl lg:text-[2.75rem] text-balance">
                  {t("One stop. One team. ")}<em className="font-display italic text-[color:var(--rose-soft)]">{t("One dream fulfilled.")}</em>
                </h2>
              </Reveal>
              <Reveal delay={0.18}>
                <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-white/70">
                  {t("From your first consultation to the moment you hold your baby, everything happens under one roof. Surgeries, IUI, IVF, ICSI, PGT, donor services, counselling, and emotional support — Bavishi Fertility Institute is the only partner you need on your journey to parenthood.")}
                </p>
              </Reveal>
              <Reveal delay={0.25}>
        ...
        <div className="relative overflow-hidden rounded-[2.5rem] gradient-dark px-8 py-16 text-center text-white noise md:px-16 md:py-20">
          <Reveal>
            <h2 className="mx-auto max-w-2xl text-3xl font-medium leading-[1.1] md:text-4xl lg:text-5xl text-balance">
              {t("Ready to begin your journey ")}<em className="font-display italic text-[color:var(--rose-soft)]">{t("with India's most trusted team?")}</em>
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mx-auto mt-6 max-w-xl text-lg text-white/70">
              {t("Book a consultation at any of our 14 centres across India. No obligation, no pressure — just honest, compassionate guidance from the pioneers of IVF in India.")}
            </p>
          </Reveal>
          <Reveal delay={0.2}>
